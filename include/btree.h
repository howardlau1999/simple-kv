...
struct KeyValue {
    ...
    bool operator<(Key const& key) const { return this->key < key; }
...
class Node {
   ...
   public:
     ...
     * \return nonnull KeyNode with key the minimum key of the splitted, node
     * the node splitted out \note The key-value will finally be inserted into
     * one leaf node.
     */
     ...
     * \return KeyNode with key the medium key to split keys of this node, node
     * the node splitted out \note you should manually insert the new node into
     * parent node of the old one.
     */
     ...
     */
    virtual bool remove(const Key& k, int index, InnerNode* parent,
                        bool& ifDelete) = 0;
...
class InnerNode : public Node {
   private:
    ...
    void getBrother(int index, InnerNode* parent, InnerNode*& leftBro,
                    InnerNode*& rightBro);
...
class Iterator {
   ...
   public:
    Iterator(const LeafNode* node, const int slot)
...
class BTree {
   private:
    ...
    void recursiveDelete(
        Node* n);  // call by the ~BTree(), delete the whole tree
